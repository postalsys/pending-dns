...
const dnsHandler = async request => {
    let did = require('crypto').randomBytes(8).toString('hex');
    console.log({ did, time: Date.now(), questions: request.questions });
    ...
    console.log({ did, time: Date.now(), questions: request.questions, answers: response.answers });
