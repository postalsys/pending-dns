...
const sessionIdContext = 'pending-dns';
...
const tlsTicketKey = id => `d:tls:${id.toString('hex')}`;

// TLS session ticket store. `newSession` writes, so it has to go to the master.
// `resumeSession` reads from the (possibly local) replica, but must not extend
// the ticket in the same transaction: EXPIRE is a write, a read-only replica
// rejects it, and that aborts the MULTI - discarding the ticket we came for and
// silently disabling session resumption on every replica-backed node. The
// refresh is a separate, best-effort write against the master.
const storeTlsTicket = (id, data, cb) => {
    const sessionKey = tlsTicketKey(id);
    db.redisWrite
        .multi()
        .set(sessionKey, data)
...
const loadTlsTicket = (id, cb) => {
    const sessionKey = tlsTicketKey(id);
    db.redisRead
        .getBuffer(sessionKey)
        .then(ticket => {
            if (ticket) {
                db.redisWrite.expire(sessionKey, 300).catch(err => {
                    logger.error({ msg: 'Failed to extend TLS ticket', ticket: id.toString('hex'), err });
                });
            }
            cb(null, ticket || null);
        })
...
const handler = async (req, res) => {
    ...
    if (/PendingDNS/.test(req.headers['x-cdn-loop'])) {
...
const setupHttps = () =>
    new Promise((resolve, reject) => {
        ...
        server.on('newSession', storeTlsTicket);
        server.on('resumeSession', loadTlsTicket);
...
module.exports.testables = { storeTlsTicket, loadTlsTicket };
