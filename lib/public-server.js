...
const handler = async (req, res, proto) => {
    ...
    const url = new URL(req.url || req.headers[':path'], `${proto}://${domain}/`);
    ...
    if (target) {
        ...
        logger.info({ msg: 'Redirect URL record', rr: target, url: url.href, redirectUrl });

        return res.end(movedTemplate);
    }

    logger.trace({ msg: 'Page not found', rr: target, url: url.href });
...
const setupHttps = () => {
    return new Promise((resolve, reject) => {
        const server = http2.createSecureServer(
            ...
            (req, res) => {
                middleware(req, res);
                handler(req, res, 'https').catch(err => {
                    res.statusCode = 500;
                    res.setHeader('Content-Type', 'text/html');
...
const setupHttp = () => {
    return new Promise((resolve, reject) => {
        const server = http.createServer((req, res) => {
            middleware(req, res);
            handler(req, res, 'http').catch(err => {
                res.statusCode = 500;
                res.setHeader('Content-Type', 'text/html');
