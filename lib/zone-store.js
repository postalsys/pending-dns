...
class ZoneStore {
    ...
    getFullId(zone, sub, type, hid) {
        return Buffer.from([zone, sub, type, hid].join('\x01')).toString('base64').replace(/\//g, '_').replace(/\+/g, '-').replace(/[=]+/g, '');
    }

    parseFullId(raw) {
        try {
            raw = Buffer.from((raw || '').toString().replace(/_/g, '/').replace(/-/g, '+'), 'base64').toString();
            let [zone, sub, type, hid] = raw.split('\x01');
            return { zone, sub, type, hid };
        } catch (err) {
    ...
    parseEntry(zone, name, zoneDomain, domain, type, short, hid, value) {
        try {
            ...
            return {
                id: this.getFullId(zone, name, type, hid),
                zone: zoneDomain,
    ...
    async add(zone, sub, type, value) {
        ...
        let res = await this.db.redisWrite.multi().hsetnx(recordKey, hid, value).sadd(`d:${zone}:z`, recordKey).exec();

        if (res[0] && !res[0][0] && res[0][1]) {
            return this.getFullId(zone, name, type, hid);
        }
    ...
    async update(id, value) {
        const { zone, name, type, hid } = this.parseFullId(id);
        if (!hid) {
            return false;
        }

        let recordKey = `d:${name}:r:${type}`;
        let res = await this.db.redisWrite.multi().hset(recordKey, hid, value).sadd(`d:${zone}:z`, recordKey).exec();
