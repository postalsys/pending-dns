'use strict';

const net = require('net');
const EventEmitter = require('events');
const { Packet } = require('dns2');
const logger = require('./logger').child({ component: 'dns-tcp-server' });

class DNSTcpServer extends EventEmitter {
    constructor(handler) {
        super();
        this.on('request', handler);

        this.server = net.createServer(socket => {
            let chunks = [];
            let chunklen = 0;
            let received = false;
            let expected = false;

            socket.setTimeout(10 * 1000);
            socket.on('timeout', () => {
                try {
                    socket.end();
                } catch (err) {
                    // ignore
                }
            });

            // assigned below; processMessage detaches it once a query is in flight
            let onReadable;

            let processMessage = () => {
                if (received) {
                    return;
                }
                received = true;

                // One query per connection: send() ends the socket once the handler
                // answers (and a packet we cannot parse ends it right below), so stop
                // collecting here. Whatever a client keeps writing until then is
                // drained and dropped by the stream instead of being buffered, which
                // would grow this connection without bound while a slow answer is
                // being built. Resuming (rather than just detaching) matters: a socket
                // nobody reads never sees EOF, so it would linger after the peer went
                // away.
                socket.off('readable', onReadable);
                socket.resume();

                let buffer = Buffer.concat(chunks, chunklen);
                // the parsed packet owns everything we still need
                chunks = [];
                chunklen = 0;

                let request;
                try {
                    request = Packet.parse(buffer.slice(2));
                } catch (err) {
                    logger.trace({ msg: 'Failed to parse DNS package', proto: 'tcp', err, buffer, port: socket.remotePort, address: socket.remoteAddress });
                    try {
                        socket.end();
                    } catch (err) {
                        // ignore
                    }
                    return;
                }

                request.source = {
                    proto: 'tcp',
                    port: socket.remotePort,
                    address: socket.remoteAddress
                };
                this.emit('request', request, message => this.send(socket, message));
            };

            onReadable = () => {
                let chunk;
                while ((chunk = socket.read()) !== null) {
                    chunks.push(chunk);
                    chunklen += chunk.length;
                }
                if (!expected && chunklen >= 2) {
                    if (chunks.length > 1) {
                        chunks = [Buffer.concat(chunks, chunklen)];
                    }
                    expected = chunks[0].readUInt16BE(0);
                }

                if (chunklen >= 2 + expected) {
                    processMessage();
                }
            };
            socket.on('readable', onReadable);

            socket.on('error', err => {
                this.emit('error', err);
            });

            socket.on('end', () => {
                processMessage();
            });
        });

        this.server.on('error', err => {
            this.emit('error', err);
        });
    }

    send(socket, message) {
        if (message instanceof Packet) message = message.toBuffer();
        return new Promise(resolve => {
            try {
                let len = Buffer.alloc(2);
                len.writeUInt16BE(message.length);

                if (['open', 'writeOnly'].includes(socket.readyState)) {
                    socket.end(Buffer.concat([len, message]));
                } else {
                    socket.end();
                }
            } catch (err) {
                // ignore
            }

            resolve(message);
        });
    }

    listen(port, address, callback) {
        this.server.listen(port, address, callback);
        return this;
    }
    close(cb) {
        this.server.close(cb);
        return this;
    }
}

const createDNSTcpServer = function (handler) {
    return new DNSTcpServer(handler);
};

module.exports = { DNSTcpServer, createDNSTcpServer };
