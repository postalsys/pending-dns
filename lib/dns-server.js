...
const init = async () => {
    ...
    createDNSUdpServer(function (request, send) {
        ...
        .listen(config.dns.port, () => {
            logger.info({ msg: 'DNS server listening', protocol: 'udp', port: config.dns.port });
        })
        .on('error', err => {
    ...
    createDNSTcpServer(function (request, send) {
        ...
        .listen(config.dns.port, () => {
            logger.info({ msg: 'DNS server listening', protocol: 'tcp', port: config.dns.port });
        })
        .on('error', err => {
