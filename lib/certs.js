...
class AcmeDNSPlugin {
    ...
    async get({ challenge }) {
        ...
        if (entry && entry.length && entry[0].value && entry[0].value.length && entry[0].value[0] === challenge.dnsAuthorization) {
            logger.trace({
    ...
    async remove({ challenge }) {
        ...
        let deleted = await zoneStore.deleteDomain(challenge.dnsHost, 'TXT', [challenge.keyAuthorizationDigest]);
        if (!deleted) {
            logger.trace({
                msg: 'No record found to delete',
                host: challenge.dnsHost,
                type: 'TXT',
                value: challenge.keyAuthorizationDigest
            });
            return false;
        }
...
const getCertificate = async (domains, force) => {
    ...
    try {
        ...
        let performRenew = async () => {
            ...
            if (!privateKey) {
                // generate new key
                logger.warn({ msg: 'Provision new private key', domains });
